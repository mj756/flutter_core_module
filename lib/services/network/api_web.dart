...
class NetworkServiceWeb implements NetworkService {
  factory NetworkServiceWeb() => _instance;
  NetworkServiceWeb._internal();
  static final NetworkServiceWeb _instance = NetworkServiceWeb._internal();
  ...
  Future<bool> hasInternet() async {
    ...
    try {
      ...
      LoggerService().log(
        message: 'error while checking error ${e.toString()}',
      );
      status = false;
    }
    return status;
    ...
    try {
      if (ConnectivityService().hasInternet == false) {
        ...
        bool hasFile = requestBody.values.any(
          (v) => v is http.MultipartFile || v is Uint8List,
        );
